function corrInfo = icatb_compute_task_dfnc_HUBAL(X, TR, varargin)
...
if (isempty(Y))
    ...
    if (task_dfnc)
        ...
        % ************** HUBAL modifications (AB) *******************
        
        %task_connectivity = zeros(size(model_tcwin, 2), size(FNCdyn, 2));
        [task_connectivity, R2] = icatb_regress(FNCdyn, model_tcwin);
        
        %model_tcwin_z = icatb_zscore(model_tcwin);
        %task_connectivity_z = icatb_regress(FNCdyn, model_tcwin_z);
        
        %  for nRegress = 1:size(model_tcwin, 2)
        %      task_connectivity(nRegress, :) = icatb_r_to_z(icatb_corr(model_tcwin(:, nRegress), FNCdyn));
        %  end
        corrInfo.model_tcwin = model_tcwin;
        corrInfo.task_connectivity = task_connectivity;
        corrInfo.task_connectivity_R2 = R2;
        %corrInfo.model_tcwin_z = model_tcwin_z;
        %corrInfo.task_connectivity_z = task_connectivity_z;
        corrInfo.XWin = XWin;
        % ************** HUBAL modifications (AB) *******************
